...
| 0x81039001 | 1 | MaxRetry (Autopilot) | [OOBE](../lifecycle/03-oobe.md) | PP, SD | Intermittent TPM attestation retry limit exceeded (E_AUTOPILOT_CLIENT_TPM_MAX_ATTESTATION_RETRY_EXCEEDED); subsequent attempt typically resolves | Retry provisioning (max 2 attempts, 15 min between); then **Escalate** — collect: serial, error code, mode, timestamp, TPM manufacturer | see [L2 TPM runbook](../l2-runbooks/03-tpm-attestation.md) |
