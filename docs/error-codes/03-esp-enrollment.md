...
| AutoAdminLogon registry = 0 | Breaks Autopilot autologon entirely; device stops at sign-in screen instead of progressing through ESP | **Escalate** — collect: serial, mode, timestamp, note "autologon fails" | Check GPO setting AutoAdminLogon = 0; exclude Autopilot devices from that GPO; [`HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon`](../reference/registry-paths.md) |
