...
| User license | User must have an active Microsoft Intune license — a standard Intune requirement, commonly overlooked as an A2 failure point |
...
| Enrollment profile device targeting | The enrollment profile is assigned to devices/serials in ABM; in Intune, assign to the same static user group as the PSSO policy and the CP LOB app — user affinity bridges the ABM device-serial assignment to the Intune user-group assignment |
