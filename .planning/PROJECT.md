...
## Current Milestone: v1.6 Apple Business Delegated Governance & Multi-Org Operations

**Goal:** Document the Apple Business rebrand and its new delegated permission surface — Locations + custom roles + granular privileges — so internal organizations can manage their own device pools (VPP catalogs, shared iPad passcode resets, device releases, MDM server assignments, account provisioning, device transfers, audit access, shared iPad / Apple TV lifecycle) without escalating to a central tenant admin, across both iOS/iPadOS and macOS through Intune.
...
---
*Last updated: 2026-05-23 — v1.6 milestone in progress: Apple Business Delegated Governance & Multi-Org Operations across iOS/iPadOS + macOS. Scope locked at one Apple Business account with OUs (formerly Locations) and/or per-team custom roles (Q2 b/c + combination); Apple Business surface only (Intune-side RBAC out of scope); rebrand handling via glossary + 2 intro callouts only (Q5 b, no corpus sweep); cross-platform iOS+macOS merged docs tree. v1.5 shipped 2026-05-07 (14 phases, 101 plans, 57/57 reqs). Phase numbering continues 61 → v1.6 spans Phases 62-66. **Progress:** Phase 62 (Foundation & Rebrand) complete; Phase 63 (Multi-OU Architecture & Apple Admin Setup) complete 2026-05-21 — OU-01..OU-10 validated (8 admin-setup docs 03-10 + 3 iOS-matrix rows + check-phase-63.mjs validator); Phase 64 (Apple Business Delegation Runbooks) complete 2026-05-22 — DELEG-01..08 validated (8 delegation runbooks 11-18: VPP catalog, shared iPad passcode reset [canonical C16 doc], device release, device transfer, single MDM-server reassign, Managed Apple Account provisioning, audit log scoping, cross-org-boundary cheat sheet + check-phase-64.mjs validator; 24/24 + C14/C15/C16 PASS); Phase 65 (Apple Business L1/L2 + Hub Navigation Integration) complete 2026-05-23 — ABNAV-01..07 validated (L1 #34 shared iPad passcode reset runbook with `#which-admin-owns-this-pool` lookup + 3-path matrix; L2 #26 permission-denied 7-leaf Mermaid hybrid tree with C15-safe Intune-scope inline leaf; 5 hub-file append-only edits — common-issues + quick-ref-l1/l2 + operations/00-index + docs/index.md; C16 4-edge cross-link triangle LIVE with zero exemptions via Wave-4 atomic-trio commit `8721a63` — 12- back-link + 4 allowlist sunset-65 removals + V-64-05 NEGATIVE→POSITIVE flip + V-62-SIDECAR co-reconciliation; check-phase-65.mjs validator-as-deliverable; full v1.6 chain green: 123 PASS / 0 FAIL across all 5 validators; PITFALL-6 anchor stability verified zero regressions). Next: Phase 66 (Apple Business Validation Tooling Closure + Milestone Audit — terminal re-audit from fresh worktree per D-22 auditor-independence).*
