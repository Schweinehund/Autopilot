...
## Current Milestone: v1.7 Deferred Backlog Closure + Validator Chain Hardening

**Status:** Executing (started 2026-05-25; Phase 67 SHIPPED 2026-05-26; Phase 68 SHIPPED 2026-05-26; Phase 69 SHIPPED 2026-05-28; Phase 70 next)

**Goal:** Close the v1.6 deferred cleanup backlog (CI-1 ABM URL surgical sweep + CI-2 VPP-location-token rename), resolve the 5-entry CHAIN_SKIP {48,51,58,60,61} root causes (CRLF regex normalization + archive-path detection + ubuntu-latest CI runner), and bump the audit harness lineage v1.6 → v1.7 with C14/C15/C16 inherited — leaving the validator chain green on both Windows AND Linux for the first time.

**Target features (4 pillars):**

- **Pillar A — Corpus surgical sweeps (low-risk)** — CI-1 ABM URL refs (4 URLs / 4 files; per `v1.6-DEFERRED-CLEANUP.md` calibrated count from sidecar); CI-2 VPP location token rename (6 occurrences / 2 files per PITFALLS.md:642); per-file edits include Version History rows + chain validator re-runs
- **Pillar B — CHAIN_SKIP root-cause resolution** — Fix CRLF regex patterns in `check-phase-{51,58}.mjs` (`\n` → `\r?\n`); fix archive-path detection in `check-phase-48.mjs`; fix line-number drift in `regenerate-supervision-pins.mjs --self-test`; cascade fix for `check-phase-{60,61}.mjs`; remove the 5 entries from CHAIN_SKIP arrays in `check-phase-62..66.mjs`; verify all phases now exit 0 on BOTH Windows AND Linux
- **Pillar C — CI-Linux hardening** — Add `ubuntu-latest` runner job to a v1.7 CI workflow that runs `check-phase-{48..N}.mjs` chain on LF line endings; coexist with existing per-version workflows (v1.4/v1.5/v1.6/v1.7); catch CRLF-style regressions in CI before they hit local
- **Pillar D — v1.7 audit harness lineage bump** — Path-A copy v1.6 → v1.7: `v1.7-milestone-audit.mjs` + `v1.7-audit-allowlist.json`; inherit C1-C13 + C14/C15/C16 + sidecar shape (`c13_rotting_external`, etc.); author `check-phase-67..70.mjs` per validator-as-deliverable pattern; CI workflow `audit-harness-v1.7-integrity.yml` as third parallel coexistence file; terminal re-audit at v1.7 close from fresh clone (D-03 pattern from v1.6); `v1.7-MILESTONE-AUDIT.md` + `v1.7-DEFERRED-CLEANUP.md` (carries CI-3 + Other Deferrals forward to v1.8+)

**Key constraints:**

- **CI-3 DEFERRED to v1.8+** — Managed Apple ID → Managed Apple Account corpus rename (45 occurrences across 16 files) waits until Microsoft Intune portal adopts the rebrand; sweeping now would create term-mismatch between docs and admin's actual portal experience
- **Other v1.6 deferrals carried forward** — Multi-tenant Apple Business / Apple Business Device API / per-OU CRD / Account Holder standalone runbook / ASM all remain in `v1.7-DEFERRED-CLEANUP.md` for v1.8+ consideration
- **Phase numbering** continues from v1.6 close at Phase 66 → v1.7 spans Phase 67+
- **Atomic harness commit pattern preserved** — Pillar D lineage bump follows v1.5 Plan 60-08 / Phase 62-08 / Phase 66-02 atomic-commit precedent
- **Sequential-on-main-tree execution** — per `.planning/config.json` `use_worktrees:false` user constraint (durable from v1.6)
- **Auditor-independence at terminal re-audit** — D-03 fresh-clone pattern from v1.6 + NEW CI-Linux independence axis (separate OS, separate host process from local Windows)
- **Estimated scope** — ~1 week elapsed time, 4 phases (67-70), ~12-18 plans
...
---
*Last updated: 2026-05-28 — v1.7 Phase 69 (Pillar C — CI-Linux Hardening / Cross-OS Verification) SHIPPED 2026-05-28. 2/2 plans complete; CILINUX-01 closed (Active → Validated). Closing SHAs: Plan 69-01 `dd1ff08` (CILINUX-01 workflow YAML NEW Path-A copy from v1.6) + Plan 69-02 Fix-1 `85521bb` (FETCH-DEPTH-01 fix: fetch-depth:0 on linux-chain-ubuntu-latest checkout) + Plan 69-02 Fix-2 `2d61981` (SCOPE-GAP-61 fix: V-61-05..08 v1.5-frozen-aware via readRoadmapAtV15Close() helper; Plan 68-03 Task 1 scope-gap closure) + Plan 69-02 close-gate (this commit). Chain-apex 28 PASS / 0 FAIL / 0 SKIPPED on BOTH Windows local AND Linux GHA ubuntu-latest (EXACT MATCH); Linux runtime ~56s (well within TIMEOUT-01 budget; 54% of Windows ~102s reference). 3 cross-OS architectural discoveries surfaced + documented in 69-VERIFICATION.md §E (FETCH-DEPTH-01 + SCOPE-GAP-61 + D-04-OVERSPEC-01); 1 latent meta-bug deferred to v1.8+ (CHAIN-WRAPPER-01: check-phase-66.mjs:313 stderr-only wrapper masked SCOPE-GAP-61 for 2 weeks). v1.7-DEFERRED-CLEANUP.md updated: TIMEOUT-01 CLOSED + FETCH-DEPTH-01 CLOSED + SCOPE-GAP-61 CLOSED + D-04-OVERSPEC-01 ACKNOWLEDGED + CHAIN-WRAPPER-01 DEFERRED. Zero-master-tree-churn invariant preserved (PR #1 closed-without-merge + branch deleted local + remote). **v1.7 progress:** 3/4 phases complete (Phase 67 + Phase 68 + Phase 69); 6/12 requirements Validated (SWEEP-01 + SWEEP-02 + CHAIN-01 + CHAIN-02 + CHAIN-03 + CILINUX-01); 10/12 plans complete across three phases (estimated). **Totals through Phase 69:** 66 v1.6 phases + 3 v1.7 phases = 69 phases, ~320 plans, ~248 documentation files. **Cross-OS verification axis now LIVE** — 3 independence axes available at Phase 70 terminal re-audit (fresh-clone + fresh sub-agent + cross-OS Linux LF). Next: `/gsd:plan-phase 70` (Pillar D — v1.7 Audit Harness Lineage Bump + Milestone Close / HARNESS-01..06).*
