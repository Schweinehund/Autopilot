...
Phase: Phase 72 ready to plan
Plan: —
Status: Phase 71 (Pillar A) closed 2026-06-04 — ARCHIVE-01 + ARCHIVE-02 Validated
Last activity: 2026-06-04 — Phase 71 closed via Plan 71-03 close-gate (chain validators 21 PASS / 8 FAIL — 8 FAILs are pre-existing chain degradation routed to v1.8-DEFERRED-CLEANUP.md CHAIN-DEGRADED-AT-HEAD-01 → Phase 73 Pillar C scope; Phase-71-OWNED validators all PASS)
...
- **v1.8 (executing): 4 phases (71-74), 12 plans estimated. Phase 71 (Pillar A) CLOSED 2026-06-04.**
...
- **Phase 72 (Pillar B — Chain-Wrapper Hardening) entry-phase planning** from `v1.7-DEFERRED-CLEANUP.md:160-192` CHAIN-WRAPPER-01 entry: surgical fix at `scripts/validation/check-phase-66.mjs:313` chain-apex wrapper to capture both `err.stdout` AND `err.stderr` (not stderr-only) — closes the 2-week-masking surface that hid SCOPE-GAP-61 on Windows local; per-validator stdout-vs-stderr audit + chain-apex regression sweep. Phase 72 entry-state ready: vendored extractor in place + MILESTONES.md clean + CHAIN_SKIP = [] preserved per Phase 71 close-gate.
- **Phase 73 (Pillar C — Retrospective Forward-Port) entry-phase planning** from `v1.7-DEFERRED-CLEANUP.md:52-66` HARNESS-FORWARD-01 + `v1.7-DEFERRED-CLEANUP.md:110-131` SCOPE-GAP-61 + Plan 71-03 close-gate `v1.8-DEFERRED-CLEANUP.md` CHAIN-DEGRADED-AT-HEAD-01 entries: RETRO-01 class-wide scan of `check-phase-{48..66}.mjs` (19 validators) for HEAD-coupled assertions + RETRO-02 per-validator conversion to v1.5/v1.6/v1.7-frozen-aware via SHA-pinned helpers parallel to existing `readRequirementsAtV15Close()` / `readRoadmapAtV15Close()` / `readCorpusFileAtV17Close()`. Empirical baseline inventory: 8 chain FAILs V-71-CHAIN-{61..67, 70} surfaced during Phase 71 chain re-runs (per CHAIN-DEGRADED-AT-HEAD-01 stub). Scope-discipline guardrail: SCOPE-GAP-class discoveries beyond initial inventory route to v1.9+ per REQUIREMENTS.md:27.
