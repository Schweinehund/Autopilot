...
- ✅ **v1.20 Frozen-Aware CI Remediation & Chain-Validator Debt Closure** (Phases 139-144) — SHIPPED 2026-08-18 — `.planning/milestones/v1.20-ROADMAP.md`
...
- [x] 147-02-PLAN.md — `00-overview.md`'s six routing sites plus the five-platform prose sweep, the three owner-ruled `REQUIREMENTS.md` markers, the `PITFALLS.md` citation fix, and the post-phase apex; commit 2 of D-59
...
**Plans**: 5/5 plans executed — 148-05 gap-closure plan pending

Plans:
**Wave 1**

- [x] 148-01-PLAN.md — fetch the six outstanding APP-01 claims, then author `07-windows-autopatch.md` tracer-first (APP-01, APP-02); commit 1 of D-65

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 148-02-PLAN.md — author `08-windows-app-updates.md` tracer-first and close the sibling link back into `07` (APP-03..APP-06); commit 2 of D-65
...
- [x] 148-04-PLAN.md — add the missing unification-article `**Source:**` line under the cadence-conflict quote pair in `08-windows-app-updates.md` (APP-03; R7 / D-63 evidence-line contract)
- [x] 148-05-PLAN.md — remove the unsupported profile-type comparison clause from `08-windows-app-updates.md`, align the `EnableMicrosoftStoreSource` table label with its ADMX name, and file WR-01/WR-02/WR-04 to the backlog (APP-03, APP-06; CR-01 / R7 evidence-line contract)
...
- [x] 149-01-PLAN.md — tracer: found `docs/operations/firmware-bios/` with BOTH files in ONE commit (D-28/D-29), then author `00-overview.md` in full at 9 H2 / 7 anchors (BIOS-01, BIOS-02, BIOS-11)
- [x] 149-02-PLAN.md — discharge the roadmap-locked glossary flag: four firmware terms plus index and version-history rows in `docs/_glossary.md` (D-06), and file the SC#4 registration-path premise amendment in `REQUIREMENTS.md` (D-05) (BIOS-02, BIOS-04)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 149-03-PLAN.md — `01-windows-dfci.md` mechanics: prerequisites and the four classified registration channels, OEM support with the nine names and the three-list conflict, and bounded Surface eligibility (BIOS-02, BIOS-03, BIOS-04); one-way decision checkpoint on the eligibility gate

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 149-04-PLAN.md — `01-windows-dfci.md` consequences: the eight-category settings surface, all three interaction traps, the corrected virtualization chain, and the bricking surface with SC#5's third clause (BIOS-02, BIOS-11)
...
- [x] 150-03-PLAN.md — the Lenovo guide: the ThinkCentre tooling fork, the model list attached to the certificate tool, an Offboarding H2 stating a structural absence, and the U-1-corrected Recovery section (BIOS-08, BIOS-06 Lenovo half)
...
- [x] 150-05-PLAN.md — the `00-overview.md` edits at seven sites and the phase's second commit: the false sentence replaced, the four outbound links, the SC#2 cross-reference sentence, the two reduced quotes, and the U-6 qualifier
...
- [x] 151-01-PLAN.md — **Commit A**: the STD-05 D-08 class widening in `EEE-SOP-standard.md` and the `## Rollback/Recovery` promotion into `recipe-template.md`; then the **tracer** — `docs/recipes/05-enterprise-update-plan.md` created at the locked eight-H2 skeleton with Decision 1 (Windows update topology, on the corrected Autopatch-containment position) authored end to end (RCP-01, RCP-02, RCP-03, RCP-05)

**Wave 2** *(blocked on Wave 1)*

- [x] 151-02-PLAN.md — Step 2 hotpatch posture on all three corrected positions with the two-level precedence rule, and Step 3 driver approval mode carrying RCP-02's single **Destructive** rating with its destroy-all-approvals consequence in the recipe's own prose (RCP-01, RCP-02, RCP-03)

**Wave 3** *(blocked on Wave 2)*

- [x] 151-03-PLAN.md — Step 4 the BIOS and firmware surface with all four branch bodies including the "none" branch and RCP-02's single **Effectively irreversible** rating, Step 5 the Microsoft 365 Apps channel rated at the recommended branch, and Step 6 the application patch mechanism (RCP-01, RCP-02, RCP-03)

**Wave 4** *(blocked on Wave 3)*

- [x] 151-04-PLAN.md — Steps 7, 8 and 9 (the three non-Windows markers, all three platform guides added to the roster) and Step 10, the tableless Case 3 deferrals-and-deadlines Step that carries an anchor but no marker and no rating (RCP-01, RCP-03)

**Wave 5** *(blocked on Wave 4)*

- [x] 151-05-PLAN.md — the four tail sections completed (eleven five-platform Verification checks; nine worst-first rollback mechanisms opening on the true count of four), the `## Rollback/Recovery` retrofit into recipes 01 and 02, the divergence entry closed at 5-of-5, and **Commit B** — the phase's single four-file content commit (RCP-01, RCP-04, RCP-05)
...
- [x] 152-01-PLAN.md — the registry atom: eleven rows at `Status: Approved`, the generator-produced filename map, both canaries bumped from measured counts (INT-01, INT-02, INT-03); Commit A, exactly four files

**Wave 2** *(blocked on Wave 1)*

- [x] 152-02-PLAN.md — the publish-bundle proof in isolation: a real run over the committed 236-row registry with an explicit version flag, computed parity assertions, no commit (INT-02, SC#3)
...
**Success Criteria** (what must be TRUE):
  ...
  2. All **six** C17-bearing harnesses (v1.15 through v1.20) are converted **simultaneously** via the working-directory swap, leaving the contract validator byte-unchanged and the existence guard live — converting a subset would leave the identical drift class on the remainder. `[SUCCESS-CRITERION AMENDMENT, D-08]` widened from five to six, OWNER-RULED 2026-08-29: v1.20's own harness — measured at seven C17 contract references against the other five harnesses' eight, the difference being exactly the `SWEEP-05 EXCEPTION` comment block — is added to the set.
...
**Plans**: 12/14 plans executed across 11 waves (sequential-on-main-tree; waves 2 and 5 carry parallel-safe siblings with zero file overlap)

**Wave 1**

- [x] 153-01-PLAN.md — TRACER: the `V120` pin inserted before `V14`, both frozen-read exports, the corrected project-document instruction, the `withDocsAtClose` materialize helper, and the first C17 leg converted and proved end to end against its own frozen corpus (HARN-01, HARN-02, HARN-03)
...
- [x] 153-03-PLAN.md — the owner-ruled six-not-five scope amendment across seven stale surfaces, the sixth harness's full corpus conversion, and the pre-conversion commit and blob hash recorded for the next plan's copy source (HARN-03)

**Wave 3** *(blocked on Wave 2)*

- [x] 153-04-PLAN.md — the 19th Path-A lineage as one indivisible unit: the new harness forked from the hash-verified unconverted source, the allowlist sidecar, and `BASELINE_25` (HARN-04)
...
- [x] 153-08-PLAN.md — content leaves for phases 151 and 152, the second transcribed verbatim from its pre-specified needle-spec with the false twin claim corrected on the record (HARN-04)

**Wave 6** *(blocked on Wave 5)*

- [x] 153-09-PLAN.md — the apex `check-phase-153.mjs` as a structural copy with a generated `[48..152]` chain, four module-load guards, three hazard fixes, the repointed harness constant, and the additive carve-out successor (HARN-04)
...
- [x] 153-11-PLAN.md — the retired carve regime with its correction of record, the four orphan artifacts committed before archival, the all-untracked hygiene measurement, the atom-branch audit, the hook sequencing, and the blocking owner push checkpoint (HARN-06)

**Wave 9** *(blocked on Wave 8)*

- [x] 153-12-PLAN.md — all 18 workflows dispatched at one shared commit with head-commit read-backs, job-level evidence keyed on four fields with every skip classified, both reproduction axes, and the single remediation-round decision (HARN-06)
...
| 153. Harness Close — V120 Pin, C17 Frozen-Aware Residue & 19th Path-A Lineage Bump | v1.21 | 12/14 | Not started |  |
