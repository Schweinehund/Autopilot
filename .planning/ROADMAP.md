...
**Success Criteria** (what must be TRUE):
  ...
  4. On a clean guard pass the pipeline emits a single versioned `docs-library-v1.17.zip` with a flat internal layout of descriptively-named `.docx` (citation title = filename) plus an in-zip manifest (RE-ID → output filename → status); the pipeline asserts **registry parity** (every Approved row appears exactly once, no missing docs, no orphan files) and logs the count so a partial bundle can never masquerade as complete
**Plans**: TBD
**UI hint**: no
