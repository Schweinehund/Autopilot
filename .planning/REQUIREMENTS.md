...
| C11 ops regex | `\bAutopatch rings\b` needs an allowlisted keyword within ±200 chars | **[CORRECTED 146 D-20/D-21 — both cited coordinates were phantoms.]** C11 is **four** patterns, not one — `\bSystem Center\b`, `\bSCCM\b[^.]*\bIntune\b`, `\bAutopatch rings\b`, `\bSafetyNet\b[^.]*\bcompliance\b`, all `gi`, scoped to every `docs/**/*.md` via a live `walkMd('docs')`. There is exactly **one** case-insensitive `\bAutopatch rings\b` hit corpus-wide — `01-windows-wufb-rings.md:65`, the `## Windows Autopatch Rings (Disambiguation)` H2 — kept green by an **intra-line** `Disambiguation` keyword at hit-offset +16, which no insertion anywhere in the file can evict. `00-overview.md` carries **zero** `Autopatch rings` hits, so the old claim about `:78` and the `mutually`/`exclusive` pair described a hit that never existed. `01` also carries two live `\bSCCM\b[^.]*\bIntune\b` hits (currently `:182` and `:187`, post-146-02 stub surgery): the first is kept by the prose word `co-management` a few characters away, and the second is kept **only** by the link path `../co-management/02-windows-workload-sliders.md` sitting inside its ±200-char window — a keyword-window keeper can live in a link path rather than in prose, and nothing else in the file signals it. Cheapest defense in new prose: write `Configuration Manager`, never `SCCM` or `System Center` — no pattern fires and no window needs maintaining. Where a hit is unavoidable, ride an allowlisted keyword and verify by running the audit, never by reading the keyword list. |
