...
| C11 ops regex | `\bAutopatch rings\b` needs an allowlisted keyword within ±200 chars | **Ordering is load-bearing.** `00-overview.md:78` is kept green **solely** by the `mutually`/`exclusive` pair. Substitute another allowlisted keyword (`disambiguation`, `co-management`, `migration`, `transition`, `PITFALL-9`) **first**, then remove the mutual-exclusivity wording — same commit. `01-windows-wufb-rings.md:77` is the safe one, independently green via its own `PITFALL-9` literal. |
