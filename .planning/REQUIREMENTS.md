...
- [x] **RETRO-04**: All **glossary docs** (`docs/_glossary-*.md`) are retrofitted to EEE — header block + `## Summary`-first + normalized Platform label — and C17 exits 0 on every glossary file before phase close. The term-surface shape (definition lists / anchor slugs) is preserved; existing plain-GitHub anchor slugs are not broken.
- [x] **RETRO-05**: All **decision-tree docs** (`docs/decision-trees/*`) are retrofitted to EEE, with their Mermaid diagrams resolved per the **Mermaid-vs-C17-#1 decision** (STD-04) — either converted to a C17-compliant text-equivalent (table / structured list) preserving every decision leaf, or handled per the permitted form the decision defines; C17 exits 0 on every decision-tree file.
- [x] **RETRO-06**: All **orphan nav-hubs** (`docs/index.md`, `docs/common-issues.md`, `docs/quick-ref-l1.md`, `docs/quick-ref-l2.md`) are retrofitted to EEE with **navigation-last discipline** (nav-hub edits committed after the content they reference); C17 exits 0 on every nav-hub file. Routing/link tables remain accurate.
- [x] **RETRO-07**: All **lifecycle docs** are retrofitted to EEE (header block + Summary-first + normalized Platform label + `Status: Approved`); C17 exits 0 on every lifecycle file. **Scope = the 22 files in the 6 canonical device/enrollment-lifecycle directories** (`docs/lifecycle/`, `docs/lifecycle-apv2/`, `docs/{android,ios,macos,linux}-lifecycle/`) — directory-enumerated, NOT glob-matched (`operations/app-lifecycle/`, `operations/patch-management/03-ios-update-lifecycle.md`, and `cross-platform/apple-business/*-lifecycle.md` are separately-deferred classes, excluded by D-08 directory precedence). **Spans Phase 121 (13 Mermaid-free files) + Phase 122 (9 Mermaid-bearing files — Mermaid resolved per STD-04, every decision leaf preserved).** Closes only when all 22 are C17-green.
...
- [x] **RETRO-09**: The **end-user Guides** (RE-175, RE-176) are retrofitted to EEE (header block + Summary-first + correct `doc_type`); C17 exits 0 on both.
...
- [x] **PIPE-04**: A **descriptive-filename normalization pass** (`V1.16-DESCRIPTIVE-FILENAME-PASS`, PIPE-02 OQ1) is defined and applied — a rename map + pipeline output-name wiring + RE-registry sync — so Copilot Studio citation titles (which derive from the SharePoint `.docx` filename, not the H1) read descriptively (e.g. `01-device-not-registered` rather than `RE-001`); the RE-NNN ↔ file mapping stays intact in the registry.
- [x] **PIPE-05**: A **Draft-label grounding probe** (`PIPE-02-DRAFT-LABEL-PROBE`, owner-run) is executed against the **shipped EEE header-block format** (`·`-separated, Platform-first) — reformat a test artifact to this shipped shape (frontmatter `status: draft` + the visible `**Status:**` block), then confirm the literal **Draft** label renders and is queryable in Copilot Studio. The v1.15 Phase-113 PIPE-02 probe **already exercised the visible `**Status:**` body-text leg** (the leg that surfaces to Copilot's semantic index — `PIPE-02-FINDINGS.md` OQ2/Q5; codified in `docs/_standards/EEE-SOP-standard.md` §"Draft status is a label, not an index gate"); the frontmatter `status:` leg is the non-surfacing one (promotes to an invisible Word custom property, per OQ4). PIPE-05 is therefore a **cosmetic format re-confirmation** of the shipped `·`-separated Platform-first block vs. the v1.15 `.`-separated Doc-ID-first stub — not a new-leg discovery. *(Discuss-flag: agent-run vs owner-run if the harness lacks live Copilot Studio access.)*
