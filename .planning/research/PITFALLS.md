...
**On Ubuntu, the reboot never happens by itself.** **[SOURCED, secondary]** `unattended-upgrades` will **never** reboot unless `Unattended-Upgrade::Automatic-Reboot "true"` is set **and** the `update-notifier-common` package is installed — which it is not on minimal installs. Sources: `ubuntu.com/server/docs/how-to/software/automatic-updates/`, `help.ubuntu.com/community/AutomaticSecurityUpdates`.
