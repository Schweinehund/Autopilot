...
The elevation limit does still bite, in its own lane: a compliance rule cannot read a patch-state file whose inspection requires elevation, so "is this device patched?" cannot be answered from a discovery script that needs elevation. **[CORRECTED 2026-08-21 — over-read defect, Phase 147 CR-02]** An earlier wording said "a root-owned patch-state file", which is wider than the source supports and would void the reboot-marker remedy this file recommends: `/run/reboot-required` is root-created but world-readable, so a user-context script reads it fine. The cited constraint is about elevation, not ownership.

**On Ubuntu, the reboot never happens by itself.** **[SOURCED, secondary]** `unattended-upgrades` will **never** reboot unless `Unattended-Upgrade::Automatic-Reboot` is turned on; Canonical documents that key with the default value `false`. Source for that half: `ubuntu.com/server/docs/how-to/software/automatic-updates/`. **[CORRECTED 2026-08-21 — attribution defect, Phase 147 CR-03]** An earlier wording quoted `Unattended-Upgrade::Automatic-Reboot "true"` as an inline literal under this attribution. That literal is **not** on the cited page: the page carries `Unattended-Upgrade::Automatic-Reboot "false";` and, for the separate modifier key, `Unattended-Upgrade::Automatic-Reboot-WithUsers "true";`. Quoting a `"true"` assignment against this key is the same same-prefix confusion this bullet already documents one sentence below. **[CORRECTED 2026-08-21 — attribution defect, Phase 147 D-32]** The second half of the original claim — that `update-notifier-common` must also be installed, and is absent on minimal installs — was credited to that same Canonical page, which does **not** carry it: `update-notifier-common` occurs **zero** times there, and the page's only `update-notifier` hit is a same-prefix **decoy** describing the graphical postpone-prompt client, an unrelated feature. The claim is **not deleted** — it is recorded as carrying **no first-party source found in Phase 147's fetch pass**; `help.ubuntu.com/community/AutomaticSecurityUpdates` is a community wiki, not first-party. Treat it as a documented silence, never as a sourced prerequisite.
