...
## 9. Open Questions / Research Flags (RESOLVED)

1. **Inventory state of `docs/l2-runbooks/00-index.md`** — **RESOLVED:** File presumed absent; not in scope for Phase 30. Phase 30 uses the `iOS L2 runbooks (Phase 31)` placeholder text (D-16 forward-promise pattern); Phase 31 will create the L2 index. **Rationale:** Runbook 16 Escalation Criteria references L2 runbooks at `../l2-runbooks/00-index.md`. Planner verified the Phase 31 placeholder category is the correct pattern per CONTEXT.md D-16 (matches Phase 28 D-22 forward-contract precedent). No pre-execution filesystem check is required — plans 30-03/04/05/06/07/09 all use the `[iOS L2 runbooks (Phase 31)](../l2-runbooks/00-index.md)` convention consistently.

2. **00-index.md APv1/APv2 anchor targets** — **RESOLVED:** Anchor `#apv1-runbooks` verified as the GitHub-slugified form of the existing H2 `## APv1 Runbooks` in `docs/l1-runbooks/00-index.md` (grep-test confirmed the H2 is present). Plan 30-01 validator Check 10 (new) asserts anchor presence at execute time. **Rationale:** Per D-26, the platform gate banner reads `[Windows L1 Runbooks](00-index.md#apv1-runbooks)`. The macOS runbooks use this exact link format at line 9 of runbook 10 — so it IS an established working anchor. HIGH confidence it's live.

3. **Phase 31 L2 runbook placeholder category** — **RESOLVED:** Plan 30-09 establishes `[iOS L2 runbooks (Phase 31)](#)` and `[iOS MAM L2 triage (Phase 31)](#)` as a new placeholder category (documented in CONTEXT.md Deferred Ideas). This pattern is applied consistently across 30-03/04/05/06/07/09 and across the retrofit enumeration (32 L2P31 resolutions in plan 30-09). **Rationale:** CONTEXT.md D-31 deferred-ideas explicitly establishes that iOS L2 placeholders (pattern: `iOS L2 runbooks (Phase 31)`) become a new placeholder category introduced by Phase 30, mirroring the Phase 28 D-22 pattern. Any L1-runbook escalation that needs L2 context references `iOS L2 runbooks (Phase 31)`, to be resolved when Phase 31 ships. This is intentionally left as forward-promise and IS compliant with project pattern.

4. **Can runbook 21 Cause B include an L1 sync trigger as a user-action?** — **RESOLVED:** NO for Cause B specifically. Cause B is actual-policy-mismatch (OS version, jailbreak, passcode). No sync trigger applies because the user must remediate device state (update iOS, change passcode, remove restricted app) — re-sync does not fix the underlying mismatch; only device remediation does. Plan 30-07 Cause B action does not include a sync step. **Rationale:** After user updates iOS, there may be a 15-minute lag before Intune re-evaluates — however, that lag is observational (wait + retry), not a sync-triggerable acceleration for policy-mismatch scenarios. The macOS precedent at runbook 12 step 6 (`Devices > macOS > [device] > Sync`) DOES use sync as an L1 action but in a profile-delivery context (not compliance evaluation). Plan 30-07 retains an OPTIONAL D-08-extension per-device-sync note cross-linking to macOS runbook 12 precedent, framed as a non-required aide — this does not extend Cause B's user-action sequence.

5. **Runbook 21 jailbreak-detection escalation — who owns the ticket?** — **RESOLVED:** SecOps / IT Security owns jailbreak tickets. Plan 30-07 Cause B jailbreak-branch escalates to Security team (NOT L2, NOT admin), and the plan specifies this explicitly in escalation criteria. **Rationale:** Standard L1 practice is security-incident escalation (not L2 troubleshooting). This is reinforced in `admin-setup-ios/06-compliance-policy.md` line 217: "Known jailbroken devices treated as compliant; data exfiltration risk open." Runbook 21 contains a dedicated Escalation Criteria branch for jailbreak routing directly to the Security team.

6. **Cross-referencing v1.2 macOS `docs/l1-runbooks/14-macos-compliance-access-blocked.md`** — **RESOLVED:** File does not currently exist in the macOS L1 set (v1.2 shipped without it). Runbook 21 Related Resources omits the cross-link; a deferred-for-v1.4 note appears in runbook 21 footer to flag the gap for a future milestone. **Rationale:** The macOS L1 set covers device/enrollment/config/compliance but not cross-platform compliance-access at `14-macos-compliance-access-blocked.md` (not authored). Cross-reference from iOS runbook 21 would introduce a broken link; deferral is the correct pattern. Consistency concerns between iOS runbook 21 and any future macOS equivalent will be addressed when the macOS runbook ships (v1.4+).
