...
**[SUCCESS-CRITERION AMENDMENT, D-24]** Annotation on V-31-29's `wc -l` line above (Phase 142-01, 2026-08-10). Two corrections, both required, neither alone sufficient: **(a) metric correction** — this line mandates `wc -l`-utility semantics; `check-phase-31.mjs`'s original implementation used `content.split('\n').length`, which is `wc -l` + 1 (measured: `wc -l` = 215/211/195/200 for runbooks 14/15/16/17 vs. the newline-split's 216/212/196/201). **(b) runbook-14 target-band re-derivation** — measured v1.3-to-HEAD retrofit delta of **+31 lines** (commits `114973ba` then `956818a0`, Phase 116-07's EEE retrofit) shifts runbook 14's documented target from roughly `160-180` to roughly `191-211`; the coded bound is recomputed by the same ±15%-per-endpoint rule the other three runbooks already satisfy, giving `[162, 242]`, which contains the measured 215 with 27 lines of headroom. Runbooks 15, 16 and 17 also grew (+21, +19, +13 respectively) so their documented targets are EQUALLY STALE but still passing under their existing bounds — deliberately left unchanged here, not re-derived. The assertion stays live: it still fails if runbook 14 collapses or explodes. Both widening the coded ceiling alone (leaving the documented target stale) and exempting the runbook from the bound entirely were considered and REJECTED — neither is what this amendment authorizes. Note: this file is referenced by a source-of-truth comment at the top of `check-phase-31.mjs` but is not read by any validator at runtime; this annotation records the drift, it does not change any check's runtime behavior by itself.
