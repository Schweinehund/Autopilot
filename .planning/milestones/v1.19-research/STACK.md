...
| **Exit-kiosk PIN (already covered)** | `exit_lock_task_mode_code` — **CORRECTED 2026-08-03 (Phase 136 Plan 1, live re-fetch of `configure-managed-home-screen`):** the "(device-restrictions-profile only)" exclusivity claim is contradicted by Microsoft's own MHS JSON worked example, which lists `exit_lock_task_mode_code` as a directly settable `managedProperty` key inside the App Configuration policy's own JSON payload. The field-level Note still directs admins to configure the password "through a device configuration profile," but that is not an exclusivity statement — both surfaces carry the key. See `136-01-SUMMARY.md` Ruling A for the full both-statements citation. + `max_number_of_attempts_for_exit_PIN`, `amount_of_time_before_try_exit_PIN_again` | GUI label "Exit lock task mode password" already documented in `05-dedicated-devices.md`; the JSON key name and the two retry-throttling keys are new detail |
