...
| 4 | admin-setup-macos/00-overview.md | RE-116 | FULL | 11 (11 nodes + 0 labeled edges, 12 plain) | LOCKED-11 | **GAP→RESOLVED** | C/D/E->F reconvergence (3 parents into Config-Failures) correctly preserved as explicit prose. BUT: node 7's real 3-way fan-out to nodes 8/9/10 (`G-->H`, `G-->I`, `G-->J`) was described in prose as a "linear Platform SSO chain, Steps 7->8->9->10->11" -- base bytes show 8 and 9 are parallel siblings of 10, not sequential predecessors. No node/edge dropped (count parity exact); topology-description mischaracterization, D-01 Rider 1 class. **RESOLVED post-verification** (orchestrator fix): the LOCKED-N annotation and the Phase-122 VH row now describe node 7's 3-way fan-out to parallel Steps 8/9/10 with Step 10→11, replacing the "linear chain" wording; count parity unchanged; C17 remains 225/0 |
