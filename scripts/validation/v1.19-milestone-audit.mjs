#!/usr/bin/env node
// v1.19 Milestone Audit Harness (Path A copy of v1.18; lineage v1.4 → v1.4.1 → v1.5 → v1.6 → v1.7 → v1.8 → v1.9 → v1.10 → v1.11 → v1.12 → v1.13 → v1.14 → v1.15 → v1.16 → v1.17 → v1.18 → v1.19; C1-C17 inherited verbatim from v1.18 [no freshness-threshold change this milestone; C17 (EEE contract) already FOLDED IN per Phase 119 D-119 — subprocess-spawn of c17-eee-contract.mjs — see check id 17; NOT re-folded this milestone])
// Source of truth: .planning/phases/138-v118-pin-17th-path-a-lineage-bump-terminal-close/138-CONTEXT.md (D-20)
// Sidecar allow-list: scripts/validation/v1.19-audit-allowlist.json (v1.19 Path-A from v1.18 — header-fields-only copy [generated + phase], all 59 line-pins across 5 arrays carried forward byte-identical; pin validity proven by the sidecar-derived pinned-file-set × `git diff --name-only 7af8a147..HEAD -- docs scripts .github` intersection, which covers all 59 pins — the pin generator's report-mode flag is explicitly NOT cited as proof, since it hardcodes the v1.7 sidecar and walks only 26 of 59 pins (correction of record: the predecessor header's claim of positive report-mode confirmation could not have proven what it claimed; see 138-CONTEXT.md D-20); no HYG-02-class line-shift event occurred this milestone)
...
//
// Usage: node scripts/validation/v1.19-milestone-audit.mjs [--verbose] [--self-test]
...
// SWEEP-05 (v1.20 Phase 144): this harness reads its corpus and its sidecar frozen at its own
// milestone-close SHA (V119 = a7bda73e, see _lib/frozen-at-close.mjs MILESTONE_CLOSE_SHAS for the
...
const MILESTONE_TAG = 'V119';
const SIDECAR_PATH = 'scripts/validation/v1.19-audit-allowlist.json'; // UNCHANGED literal -- GOV-02
...
const checks = [
  ...
  },
  {
    ...
    // into v1.19 (this milestone does not re-fold or re-derive the contract).
    ...
    run() {
      ...
      // V119's own close SHA is materialized into a temp directory and the byte-unchanged
      ...
      try {
        return withDocsAtClose('V119', (tmpDir, writtenPaths) => {
          ...
          // read as a silent, vacuous PASS. [MEASURED 2026-08-29] docs/recipes/03-windows-11-multi-app-kiosk.md
          // was added at the V118->V119 boundary (absent from predecessor V118, present in V119 and
          // forward). V119 and V120 share a byte-identical docs/ path list (sha1 of sorted
          // git ls-tree -r --name-only <sha> -- docs output matches) -- no path is absent from V119's
          // immediate successor V120, mirroring the plateau Plan 153-01 found for V115. This guard
          // distinguishes V119 from a regression to V118 or earlier, the realistic wrong-milestone-tag
          // failure; it structurally cannot distinguish V119 from V120 by path presence -- no such
          // path exists, a Rule 1 deviation recorded in the plan summary.
          const KNOWN_MEMBER = join(tmpDir, 'docs', 'recipes', '03-windows-11-multi-app-kiosk.md');
          if (!existsSync(KNOWN_MEMBER)) {
            throw new Error('C17 FAIL: known-member guard failed -- ' + KNOWN_MEMBER + ' absent from materialized V119 corpus');
          }
          ...
          try {
            ...
            return { pass: true, detail: 'c17-eee-contract.mjs exits 0 against frozen V119 corpus (' + writtenPaths.length + ' files materialized; all enrolled files pass 13 assertions)' };
          } catch (err) {
