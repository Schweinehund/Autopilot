#!/usr/bin/env node
// check-phase-87.mjs -- Phase 87 deliverables (navigation hub)
//
// v1.10 per-phase validator. Phase 87 shipped its content/corpus already covered by the
...
//
// Assertion classes:
//   V-87-SELF       CHAIN_PHASES does NOT include 87 AND CHAIN_SKIP is empty Set (dual-invariant)
...
//
// Usage: node scripts/validation/check-phase-87.mjs [--verbose]
...
checks.push({
  ...
  run() {
    if (CHAIN_PHASES.includes(87)) {
      return { pass: false, detail: 'CHAIN_PHASES includes 87 -- self-reference regression' };
    }
    if (CHAIN_SKIP.size !== 0) {
      const skipList = [...CHAIN_SKIP].join(',');
      return { pass: false, detail: 'CHAIN_SKIP non-empty (' + skipList + ') -- Phase 68 7b635ca empty-Set invariant violated' };
    }
    return { pass: true, detail: 'CHAIN_PHASES = [' + CHAIN_PHASES.join(',') + '] (87 absent); CHAIN_SKIP = [] (Phase 68 7b635ca invariant)' };
  }
});
...
console.log('check-phase-87 -- Phase 87 deliverables (navigation hub)\n');
