#!/usr/bin/env node
// v1.17 Milestone Audit Harness (Path A copy of v1.16; lineage v1.4 → v1.4.1 → v1.5 → v1.6 → v1.7 → v1.8 → v1.9 → v1.10 → v1.11 → v1.12 → v1.13 → v1.14 → v1.15 → v1.16 → v1.17; C1-C17 inherited verbatim from v1.16 [no freshness-threshold change this milestone; C17 (EEE contract) already FOLDED IN per Phase 119 D-119 — subprocess-spawn of c17-eee-contract.mjs — see check id 17; NOT re-folded this milestone])
// Source of truth: .planning/phases/128-v116-pin-15th-path-a-lineage-bump-terminal-close/128-CONTEXT.md (D-128-A..D-128-D)
// Sidecar allow-list: scripts/validation/v1.17-audit-allowlist.json (v1.17 Path-A from v1.16 — docs/_glossary-android.md + docs/reference/android-capability-matrix.md + docs/admin-setup-android/03-fully-managed-cobo.md + docs/android-lifecycle/03-android-version-matrix.md C1/C2 line-pins (35 total) TARGETED -1-line-shift-repointed per the HYG-02 4-file phase_46_wave2_retrofit frontmatter-line deletion (commit 7dda1f7); confirmed against live corpus, not copied verbatim; docs/admin-setup-android/04-byod-work-profile.md (count-based tracker, no line-pins) and all OTHER entries remain byte-unchanged since v1.16 close and are copied verbatim)
...
//
// Usage: node scripts/validation/v1.17-milestone-audit.mjs [--verbose] [--self-test]
...
// milestone-close SHA (V117 = b56bba5, see _lib/frozen-at-close.mjs MILESTONE_CLOSE_SHAS for the
...
const MILESTONE_TAG = 'V117';
const SIDECAR_PATH = 'scripts/validation/v1.17-audit-allowlist.json'; // UNCHANGED literal -- GOV-02
...
const checks = [
  ...
  },
  {
    ...
    // into v1.17 (this milestone does not re-fold or re-derive the contract).
    ...
    run() {
      ...
      // V117's own close SHA is materialized into a temp directory and the byte-unchanged
      ...
      try {
        return withDocsAtClose('V117', (tmpDir, writtenPaths) => {
          ...
          // output matches across all three) -- V117 has zero paths absent from its immediate
          // predecessor V116 (they are the same plateau). docs/_templates/reference-template.md was
          // added at the V114->V115 boundary (absent from predecessor V114, present throughout the
          // V115/V116/V117 plateau) and is used here as the guard, an independent path literal from
          // V116's. It distinguishes V117 from a regression to V114 or earlier, the realistic
          // wrong-milestone-tag failure; it structurally cannot distinguish V117 from V115 or V116 by
          // path presence -- no such path exists, a Rule 1 deviation recorded in the plan summary.
          const KNOWN_MEMBER = join(tmpDir, 'docs', '_templates', 'reference-template.md');
          if (!existsSync(KNOWN_MEMBER)) {
            throw new Error('C17 FAIL: known-member guard failed -- ' + KNOWN_MEMBER + ' absent from materialized V117 corpus');
          }
          ...
          try {
            ...
            return { pass: true, detail: 'c17-eee-contract.mjs exits 0 against frozen V117 corpus (' + writtenPaths.length + ' files materialized; all enrolled files pass 13 assertions)' };
          } catch (err) {
