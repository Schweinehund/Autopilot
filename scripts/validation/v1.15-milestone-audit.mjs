...
// Sidecar allow-list: scripts/validation/v1.15-audit-allowlist.json (v1.15 Path-A from v1.14 — docs/reference/android-capability-matrix.md + docs/admin-setup-android/03-fully-managed-cobo.md + docs/l2-runbooks/20-android-app-install-investigation.md + docs/admin-setup-android/07-knox-mobile-enrollment.md + docs/admin-setup-android/02-zero-touch-portal.md C2/C7/C9 line-pins repointed per Phase-118 EEE-retrofit drift; confirmed against live corpus, not copied verbatim)
...
//   C16: 4-edge cross-link integrity triangle (BLOCKING from Phase 62 D-A9)
//   C17: EEE document contract — 13 assertions on all enrolled docs/ files (BLOCKING; FOLDED IN Phase 119 D-119 via subprocess-spawn of c17-eee-contract.mjs)
//
// Usage: node scripts/validation/v1.15-milestone-audit.mjs [--verbose] [--self-test]
...
// milestone-close SHA (V115 = 29a3599, see _lib/frozen-at-close.mjs MILESTONE_CLOSE_SHAS for the
...
const MILESTONE_TAG = 'V115';
const SIDECAR_PATH = 'scripts/validation/v1.15-audit-allowlist.json'; // UNCHANGED literal -- GOV-02
...
const checks = [
  ...
  },
  {
    ...
    // (no exports) and its own header names Phase 119 as the fold-consumer. This is the first milestone
    // where the Path-A harness gates a corpus-wide STRUCTURAL contract, not just terminology/link checks.
    ...
    run() {
      ...
      // Phase 153 conversion (D-11/D-16, tracer): the frozen docs/ tree at V115's own close SHA is
      // materialized into a temp directory and the byte-unchanged c17-eee-contract.mjs is spawned
      // with that directory as its cwd -- a working-directory swap, not a code change. The prior
      // Phase 140 deferral comment declaring this leg intentionally live-HEAD is now false and has
      // been removed; this leg reads its own frozen corpus like every other check above it.
      try {
        return withDocsAtClose('V115', (tmpDir, writtenPaths) => {
          ...
          // read as a silent, vacuous PASS. [MEASURED 2026-08-29] docs/_standards/EEE-SOP-standard.md
          // was added in v1.15 (absent from predecessor V114's docs/ tree) and V116's docs/ path set
          // is unchanged from V115's (zero added, zero removed) -- so no path is absent from BOTH
          // immediate neighbours at once; a count cannot do this job either, since enrolled counts
          // collide across milestones while the trees genuinely differ. This guard distinguishes
          // V115 from its predecessor V114, the neighbour a milestone-tag regression would
          // realistically materialize instead.
          const KNOWN_MEMBER = join(tmpDir, 'docs', '_standards', 'EEE-SOP-standard.md');
          if (!existsSync(KNOWN_MEMBER)) {
            throw new Error('C17 FAIL: known-member guard failed -- ' + KNOWN_MEMBER + ' absent from materialized V115 corpus');
          }
          ...
          try {
            ...
            return { pass: true, detail: 'c17-eee-contract.mjs exits 0 against frozen V115 corpus (' + writtenPaths.length + ' files materialized; all enrolled files pass 13 assertions)' };
          } catch (err) {
