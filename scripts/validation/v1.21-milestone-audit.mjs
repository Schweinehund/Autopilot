#!/usr/bin/env node
// v1.21 Milestone Audit Harness (Path A copy of v1.20 -- the NINETEENTH Path-A lineage bump, a
// different ordinal from the EIGHTEENTH frozen-aware-conversion sequence that landed on v1.20's own
// on-disk form one plan earlier in this same phase [153-03]; lineage v1.4 → v1.4.1 → v1.5 → v1.6 →
// v1.7 → v1.8 → v1.9 → v1.10 → v1.11 → v1.12 → v1.13 → v1.14 → v1.15 → v1.16 → v1.17 → v1.18 →
// v1.19 → v1.20 → v1.21; C1-C17 inherited verbatim from v1.20 [no freshness-threshold change this
// milestone; no C18 folded in -- new corpus content is already gated by C17, the eight Phase 153
// content leaves and the link checker, per D-44; C17 (EEE contract) already FOLDED IN per Phase 119
// D-119 — subprocess-spawn of c17-eee-contract.mjs — see check id 17; NOT re-folded this milestone];
// 16 checks registered [ids 1-7, 9-17 -- C8 was never defined, counted from the live checks array,
// not carried from any predecessor header prose])
// Source of truth: .planning/phases/153-harness-close-v120-pin-c17-frozen-aware-residue-19th-path-a/153-CONTEXT.md (D-44, D-45, D-46)
// Residue (D-46): this harness is BORN LIVE-HEAD, including its C17 leg, because there is no V121
// close SHA for the current milestone to freeze against -- the back-anchor invariant is that a pin
// always references a PAST close, and v1.21 has not closed yet. It therefore becomes the successor
// (v1.22) milestone's SOLE unconverted harness, exactly as v1.20 was this milestone's sole
// unconverted harness before 153-03 converted it. This residue is tracked in
// .planning/milestones/v1.21-DEFERRED-CLEANUP.md so the pattern does not silently repeat unrecorded.
// Sidecar allow-list: scripts/validation/v1.21-audit-allowlist.json (v1.21 Path-A from v1.20's
// sidecar — header-fields-only copy [generated + phase], all pin arrays across 5 top-level arrays
// plus the nested rotting-external object carried forward byte-identical, per D-47; a naive
// top-level walk undercounts the pinned file set by roughly half because the rotting-external
// section is an object of nested arrays, so any file-set derivation must recurse -- see Task 2's
// summary for the recursive-vs-naive count). Pin-drift proof against a back-anchor SHA is NOT
// re-run for this generation: D-46's residue applies here too -- there is no V121 close SHA yet to
// diff against, so no back-anchor pin-drift measurement is claimed in this header. When one becomes
// possible at the next milestone close, it follows the same method v1.20's header recorded: intersect
// the recursively-derived pinned file set with the changed-file set since the back-anchor SHA and
// adjudicate the resulting hunks line-granularly -- never the pin generator's report-mode instrument,
// which hardcodes the v1.7 sidecar and walks fewer than half the pins (D-48).
...
// C10 (Linux frontmatter): BLOCKING from Phase 48 -- new Linux docs must have platform: Linux + 90d last_verified (v1.14 D-01a cadence supersedes 60d).
...
//   C5: last_verified frontmatter on all Android docs (review_by - last_verified <= 90 days per Phase 112 discuss-flag #7 / 101-CONTEXT 90-day lock; supersedes Phase 34 D-14 60d)
...
//   C16: 4-edge cross-link integrity triangle (BLOCKING from Phase 62 D-A9)
//   C17: EEE document contract — 13 assertions on all enrolled docs/ files (BLOCKING; inherited from v1.15, FOLDED IN Phase 119 D-119 via subprocess-spawn of c17-eee-contract.mjs)
//
// Usage: node scripts/validation/v1.21-milestone-audit.mjs [--verbose] [--self-test]
...
function parseAllowlist() {
  const raw = readFile('scripts/validation/v1.21-audit-allowlist.json');
...
const checks = [
  ...
  },
  {
    ...
    // and review_by - last_verified must be <= 90 days (Phase 112 v1.14 90-day cadence per
    // discuss-flag #7 / 101-CONTEXT 90-day lock; supersedes the Phase 34 D-14 60-day Android cadence).
    run() {
      const violations = [];
      for (const relPath of androidDocPaths()) {
        const content = readFile(relPath);
  ...
  {
    id: 10,
    name: 'C10: Linux frontmatter (platform: Linux + 90d last_verified)',
    ...
    // review_by present; review_by - last_verified <= 90 days (Phase 112 v1.14 90-day cadence per
    // discuss-flag #7; supersedes the Phase 34 D-14 60-day cadence for Linux too).
  ...
  },
  {
    ...
    // Phase 119 fold (D-119): c17-eee-contract.mjs (authored Phase 115 HARN-01) is self-executing
    // (no exports) and its own header names Phase 119 as the fold-consumer. Inherited byte-unchanged
    // into v1.16 (this milestone does not re-fold or re-derive the contract).
    // Subprocess-spawn (argument-array execFileSync — never a concatenated shell string; the project's one
    // process-level security control) mirrors check-phase-112.mjs's existing AUDIT-HARNESS pattern exactly:
    // spawn a separate self-tested validator, treat its exit code as pass/fail. NOT an inline copy of the
