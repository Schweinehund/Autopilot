#!/usr/bin/env node
// v1.18 Milestone Audit Harness (Path A copy of v1.17; lineage v1.4 → v1.4.1 → v1.5 → v1.6 → v1.7 → v1.8 → v1.9 → v1.10 → v1.11 → v1.12 → v1.13 → v1.14 → v1.15 → v1.16 → v1.17 → v1.18; C1-C17 inherited verbatim from v1.17 [no freshness-threshold change this milestone; C17 (EEE contract) already FOLDED IN per Phase 119 D-119 — subprocess-spawn of c17-eee-contract.mjs — see check id 17; NOT re-folded this milestone])
// Source of truth: .planning/phases/134-v117-pin-16th-path-a-lineage-bump-terminal-close/134-CONTEXT.md (GA-1..GA-4)
// Sidecar allow-list: scripts/validation/v1.18-audit-allowlist.json (v1.18 Path-A from v1.17 — byte-verbatim copy, zero pin-coordinate drift positively confirmed via `regenerate-supervision-pins.mjs --report` against the live v1.18 corpus per Plan 134-02 Task 1; no HYG-02-class line-shift event occurred this milestone)
...
//
// Usage: node scripts/validation/v1.18-milestone-audit.mjs [--verbose] [--self-test]
...
const checks = [
  ...
  },
  {
    ...
    // into v1.18 (this milestone does not re-fold or re-derive the contract).
    ...
    run() {
      ...
      // V118's own close SHA is materialized into a temp directory and the byte-unchanged
      ...
      try {
        return withDocsAtClose('V118', (tmpDir, writtenPaths) => {
          ...
          // read as a silent, vacuous PASS. [MEASURED 2026-08-29] docs/recipes/01-shared-windows-avd-client.md
          // was added at the V117->V118 boundary (absent from predecessor V117, present in V118 and
          // forward). docs/ growth in this history is monotonic add-only with zero removals V114
          // through V120, so V119 is a strict superset of V118's path list -- no path is absent from
          // BOTH neighbours simultaneously, mirroring the plateau Plan 153-01 found for V115. This
          // guard distinguishes V118 from a regression to V117 or earlier, the realistic
          // wrong-milestone-tag failure; it structurally cannot distinguish V118 from its successor
          // V119 by path presence -- no such path exists, a Rule 1 deviation recorded in the plan
          // summary.
          const KNOWN_MEMBER = join(tmpDir, 'docs', 'recipes', '01-shared-windows-avd-client.md');
          if (!existsSync(KNOWN_MEMBER)) {
            throw new Error('C17 FAIL: known-member guard failed -- ' + KNOWN_MEMBER + ' absent from materialized V118 corpus');
          }
          ...
          try {
            ...
            return { pass: true, detail: 'c17-eee-contract.mjs exits 0 against frozen V118 corpus (' + writtenPaths.length + ' files materialized; all enrolled files pass 13 assertions)' };
          } catch (err) {
