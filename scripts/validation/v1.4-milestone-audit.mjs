...
// SWEEP-05/SWEEP-08 (v1.20 Phase 140): this harness reads its corpus and its sidecar frozen at
// its own milestone-close SHA (V14 = 0b3be9ab, see _lib/frozen-at-close.mjs MILESTONE_CLOSE_SHAS
// for the pin rationale) instead of live HEAD, resolving the frozen-vs-evolved freshness mismatch.
const MILESTONE_TAG = 'V14';
const SIDECAR_PATH = 'scripts/validation/v1.4-audit-allowlist.json'; // UNCHANGED literal -- GOV-02
...
// Returns a de-duplicated, sorted array of relative paths that exist at the frozen SHA.
function androidDocPaths() {
  const paths = new Set();

  // Root singletons (activate only when they exist at the frozen SHA)
  for (const p of [
