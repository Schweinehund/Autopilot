...
function parseInventory() {
  ...
  if (relPath === null) return { _missing: true, placeholders: [] };
...
const checks = [
  ...
  { id: 21, name: "V-31-21: Zero 'Phase 31' placeholder mentions across 9 retrofit targets", type: "link-graph", required: true,
    run() { const inv = parseInventory(); if (inv._missing) return { pass: false, detail: 'placeholder-inventory.json not resolvable at .planning/phases/ or .planning/milestones/v1.3-phases/' }; if (inv._parseError) return { pass: false, detail: 'inventory JSON invalid: ' + inv._parseError }; const offenders = []; for (const p of inv.placeholders) { const c = readFile(p.file); if (!c) continue; const scrubbed = c.replace(/Resolved Phase 31 L2 cross-references/g, ''); if (/Phase 31/.test(scrubbed)) { offenders.push(p.file); } } return { pass: offenders.length === 0, detail: offenders.length ? `Phase 31 still in: ${[...new Set(offenders)].join(', ')}` : "0 remaining" }; } },
  ...
  // V-31-29: '17' lower bound widened 187→170 for safety margin (IN-06). Bound re-derived
  // per D-24(b): documented target ~160-180 shifted +31 (the MEASURED v1.3->HEAD EEE-retrofit
  // delta, 114973ba then 956818a0) to ~191-211, then +/-15% per endpoint per
  // 31-VALIDATION.md:69 (0.85x191=162.35->162, 1.15x211=242.65->242) -> [162, 242], which
  // contains the file's real measured count (215) with +27 headroom. Metric corrected to
  // wc -l, which that spec mandates -- split('\n').length was exactly one higher. Runbooks
