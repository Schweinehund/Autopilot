...
This guide walks administrators through creating the [ADE](../_glossary-macos.md#ade) token that connects Microsoft Intune to [Apple Business Manager](../_glossary-macos.md#abm), assigning devices to your MDM server, and setting up the default enrollment profile. Complete these steps before configuring an enrollment profile or deploying any macOS devices.
...
- Managed Apple ID (NOT a personal Apple ID)

> **Warning:** Using a personal Apple ID means the [ABM Token](../_glossary-macos.md#abm-token) cannot be renewed if that employee leaves the organization. Always use a Managed Apple ID tied to a shared organizational role.
...
> **What breaks if misconfigured:** Device must be assigned BEFORE first power-on. If powered on without assignment, the device proceeds through non-managed [Setup Assistant](../_glossary-macos.md#setup-assistant). Requires factory wipe to fix. Symptom appears in: the device itself (standard macOS setup, no MDM enrollment).
> See: [Troubleshooting Runbook](TBD - Phase 24)

> **What breaks if misconfigured:** Wrong MDM server selected in multi-server organizations (test vs. production). Verify at **Devices** > **[serial]** > **Edit MDM Server**. Symptom appears in: wrong Intune tenant or enrollment profile applied.
> See: [Troubleshooting Runbook](TBD - Phase 24)

**Additional assignment pitfalls:**

- **Device enrolled in another MDM:** The previous organization must release the device in ABM before it can be reassigned to your MDM server.
- **Non-ABM-linked resellers:** Devices purchased from a reseller not linked to your ABM account do not appear in ABM. Contact the reseller to establish the ABM relationship, or use Apple Configurator (physical access required).
...
> **What breaks if misconfigured:** If no enrollment profile is assigned before the device first powers on, the device boots through standard (non-managed) Setup Assistant. Requires wipe to fix. Symptom appears in: the device itself (standard macOS setup) and Intune admin center (device does not appear in Devices > macOS).
> See: [Troubleshooting Runbook](TBD - Phase 24)
...
> **Note:** Up to 3,000 ADE tokens are supported per Intune tenant; each token corresponds to one MDM server in ABM.
...
> **Operational tip:** Set calendar reminders 30 days before ADE token and APNs certificate expiration. The Intune admin center shows expiration dates on the Enrollment program tokens blade and the Apple push notification certificate page respectively.
