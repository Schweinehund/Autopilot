...
1. **No security baselines** -- admins must manually configure all security settings via Settings Catalog and compliance policies
2. **No app dependency or install order control** -- critical for complex deployments with runtime prerequisites
3. **No app supersedence** -- cannot automatically replace old app versions with newer ones
4. **No pre-provisioning (White Glove)** -- IT cannot pre-stage devices before user delivery
5. **Simpler but less granular Await Configuration** vs Windows ESP -- no per-app progress tracking, no configurable timeout
6. **Apps not removed on device retirement** -- explicit uninstall required before retiring a device
7. **No userless device compliance** -- compliance evaluation requires user affinity for macOS devices

macOS does have capabilities that Windows lacks in Intune: Privacy Preferences Policy Control (PPPC), Platform SSO (macOS 14+), Kernel Extension management, and Declarative Device Management (DDM) for software updates. These are platform-specific features, not parity gaps.
...
- [macOS App Deployment](../admin-setup-macos/04-app-deployment.md) -- detailed app deployment guide with comparison table

---

| Version | Change | Author |
|---------|--------|--------|
| 2026-04-14 | Initial version -- 5-domain capability matrix with key gaps summary | -- |
