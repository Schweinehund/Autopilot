...
Intune supports three macOS app deployment types: DMG, PKG (managed and unmanaged), and VPP/Apps and Books. Each type has different size limits, detection rules, agent requirements, and uninstall capabilities. Use the comparison table below to select the correct deployment type, then follow the per-type instructions.
...
- Devices enrolled via ADE (see [ABM Configuration](01-abm-configuration.md) for enrollment setup)
- Intune Management Extension (IME) agent installed on target devices (required for DMG and unmanaged PKG)
...
- Apple Business Manager account with active VPP location token (see [ABM Configuration](01-abm-configuration.md) for token setup)
...
- Annual token renewal configured (see [Renewal / Maintenance](#renewal--maintenance) section below)
...
2. Navigate to **Apps** > **macOS** > **Add** > **macOS app (DMG)**.
3. Select the .dmg app package file (max 8 GB).
4. Configure app information (name, description, publisher).
5. Configure the **Included apps** list: add the `.app` bundle(s) contained in the DMG. The first app in the list is used for reporting.
6. Configure detection rules: Bundle ID + build number from Included apps.
7. Configure assignments: Required, Available, or Uninstall.
...
1. Sign in to [Intune admin center](https://intune.microsoft.com).
2. Navigate to **Apps** > **macOS** > **Add** > **macOS app (PKG)** (select "Line-of-business app").
3. Upload the `.pkg` file directly (max 2 GB). No `.intunemac` wrapping required (wrapping was removed August 2022).
4. Signing requirement: must be signed with "Developer ID Installer" certificate.
5. Configure "Install as managed" setting: set to **Yes** for macOS 11+ if Uninstall support is needed.
6. Configure Included apps for detection (Bundle ID + version).
7. Assign to groups: Required, Available for enrolled devices, or Uninstall (when Install as managed is Yes).
...
> **What breaks if misconfigured:** PKG without payload (e.g., script-only packages) continuously reinstalls because Intune cannot detect successful installation. Symptom appears in: Intune admin center (perpetual "Installing" status) and on device (repeated installation prompts).
> See: [Troubleshooting Runbook](TBD - Phase 24)
...
1. Sign in to [Intune admin center](https://intune.microsoft.com).
2. Navigate to **Apps** > **macOS** > **Add** > **macOS app (PKG)** (Intune auto-detects based on signing).
3. Upload `.pkg` file directly (max 8 GB). Signed OR unsigned packages supported.
4. Agent requirement: IME version 2308.006+ required.
5. Pre/post-install scripts supported (agent 2309.007+ required).
6. Assign: Required or Available (no Uninstall -- Known Issue).
...
> **What breaks if misconfigured:** Uninstall assignment type is not available for unmanaged PKG (Known Issue in Intune). If uninstall is required, use DMG or managed LOB PKG type instead. Symptom: Uninstall option missing from assignment type dropdown.
...
3. Verify the VPP location token is active and synced.
4. Navigate to **Apps** > **macOS** -- VPP apps appear automatically after sync.
5. Select the app > **Properties** > **Assignments**.
6. Assign as Required (device or user groups) or Available for enrolled devices (user groups only).
...
Up to 3,000 VPP tokens are supported per Intune tenant. One VPP token cannot be shared between Intune tenants.
