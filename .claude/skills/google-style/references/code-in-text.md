...
_Source: https://developers.google.com/style/code-in-text_

In ordinary text sentences (as opposed to, say, [code samples](code-samples.md)), use code font to mark up most things that have anything to do with code. Code font helps to clarify for your reader which text refers to an entity in these ways:
...
For information about choosing HTML or Markdown, see [Markdown versus HTML](markdown.md).
...
- [Formatting placeholders](placeholders.md)
- [Documenting command-line syntax](code-syntax.md)
- [Code samples](code-samples.md)
- [Code style guides](code-samples.md#coding)
- [Formatting a heading or title](headings.md#formatting-a-heading-or-title)
...
[Command-line utility names](#tool-names), such as `gcloud`, `gsutil`, `kubectl`, and `bq`
...
Filenames, [filename extensions](filenames.md#file-type-names) (if used), and paths
...
[HTTP content-type](https://www.w3.org/Protocols/rfc1341/4_Content-Type.html) values
...
[Placeholder variables](placeholders.md)

Replace `SUBNETWORK_NAME` with the resource ID of the private subnet that you want the blueprint to use.

Package names

The Beautiful Soup library for parsing web pages is distributed as the `beautifulsoup4` package.
...
[UI elements](ui-elements.md) that are rendered based on previously entered text (such as a server or instance name)

From the **Server name** list, select **`my-sql-cluster1`**.

Click **`my-instance`**.

If a code-formatted element appears in UI, add bold as well. For more information, see [Code in UI elements](#code-in-ui).
...
It's usually best to format a URL as a link and use descriptive link text instead of exposing the URL itself. For more information, see [Avoid URLs as link text](cross-references.md#urls).

## Code in UI elements

If a [UI element](ui-elements.md#formatting) meets the [requirements for code font](#code), then use both code font and bold for that element.
...
: To retrieve the zebra's metadata, call its `animal.get` method.
...
<p>You can't call the <code translate="no" dir="ltr">close</code> method for a file before you call <code translate="no" dir="ltr">open</code>.</p></td>
...
<tr>
<td><p>Takes an array of extended ASCII code points (an array of <code translate="no" dir="ltr">INT64</code> values) and returns <code translate="no" dir="ltr">BYTES</code> values.</p>
<p>For <code translate="no" dir="ltr">STRING</code> arguments, returns the original string with all alphabetic characters in uppercase.</p></td>
<td>Takes an array of extended ASCII code points (ARRAY of INT64) and returns BYTES.</td>
</tr>
</tbody>
</table>

## Android: Linking API terms in Android
...
: The [`Activity` class](https://developer.android.com/reference/android/app/Activity.html) is an important part of an application's overall lifecycle...

: The user interface for an activity is provided by a hierarchy of views—objects derived from the [`View` class](https://developer.android.com/reference/android/view/View.html).
