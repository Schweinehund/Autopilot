...
_Source: https://developers.google.com/style/inclusive-documentation_
...
Be mindful of the [pronouns](pronouns.md#gender-neutral-pronouns) that are used in narrative examples, and be aware of other possible sources of gendered language.
...
- Avoid [jargon](jargon.md).
- Use terms that are established industry standards and widely understood by the target audience.

When you try to achieve a [friendly and conversational tone](tone.md), you might mistakenly use figurative language. Figurative language can come in the form of figures of speech and other turns of phrase. Be attentive to your word choice, especially when you aim for an informal tone.

Don't use metaphors, and don't use a term in a metaphorical sense ([use words in their primary sense](translation.md#use-words-in-their-primary-sense)). For example, avoid using the metaphor of *pets versus cattle* when you compare on-premises or stateful systems with stateless cloud systems.

For guidance about specific terms, see the [Word list](word-list.md).
...
| Replace the placeholder in this example with the appropriate value. | Replace the [dummy variable](word-list.md#dummy-variable) in this example with the appropriate value. |

### <span id="violent-language"></span>Avoid graphic or metaphorical language

Avoid unnecessarily graphic or metaphorical language, when you can use a more precise term.

For example, instead of *[STONITH](word-list.md#stonith)*, use specific terms to describe the process that's used to stop an errant node. If you need to mention a term such as *STONITH*, you can mention it once when you first explain the relevant feature, and phrase it in a way that de-emphasizes the term.

: This approach might require you to fence failed nodes.

: This approach might require you to fence failed nodes (sometimes referred to as *STONITH*).

Always use the most precise and well-understood terms for your context. In some contexts, an industry-established term has a specific technical meaning that doesn't have an accurate synonym or alternative. For examples, see the word list entries for [terminate](word-list.md#terminate) and [execute](word-list.md#execute).
...
Write documentation for a [global audience](translation.md#be-inclusive). Use diverse names, genders, ages, and locations in examples. Keep the following advice in mind:

- Follow our [gender-neutral pronoun](pronouns.md#gender-neutral-pronouns) guidance.
- Avoid being too culturally specific to the US. Be mindful when referring to specific holidays (see also the word list entry for [*the holidays*](word-list.md#holiday)), cultural practices, sports, and figures of speech.
- In examples, [choose a diverse set of names](examples.md#example-person-names) to help reflect our global audience. For guidelines about fictional people, see [Further notes about example people](examples.md#further-notes-about-example-people).
...
Avoid using socially charged terms for technical concepts where possible. For example, avoid terms such as [blacklist](word-list.md#blacklist) and [native](word-list.md#native) feature, and don't use terms like [first-class citizen](https://wikipedia.org/wiki/First-class_citizen), even though such terms might still be widely used.

### Replace or write around non-inclusive terms

This section contains guidance about how to replace or write around a non-inclusive term. If a term is well established in the industry and replacing it could cause confusion, see [Replace established terms](#replace). If a term occurs in code samples or keywords, see [Write around non-inclusive code terms](#write-around). For information about avoiding non-inclusive jargon, see [Jargon](jargon.md).
...
: In this model, a Jenkins controller (master) handles HTTP requests. The Jenkins controller is designed to ...

: In cloud architecture, servers are treated as commodities (sometimes described by using the metaphor *cattle, not pets*).
...
: You can allow requests from a range of IP addresses by entering a CIDR block instead of a single address in the field.

: You can allowlist a range of IP addresses by entering a CIDR block instead of a single address in the field.
...
: The configuration file helps you create a parent node (which is named `master` in the file).

: Start the replica by using the `START SLAVE` statement.
