...
_Source: https://developers.google.com/style/placeholders_
...
**Note**: You can mark up command-line syntax with [brackets](code-syntax.md#optional-arguments), [braces](code-syntax.md#set-of-two-arguments), and [ellipses](code-syntax.md#arguments-that-can-repeat). Don't put the brackets, braces, or ellipses in the `var` element.
...
- Tag each placeholder in a code sample or command with `code` and `var` elements, followed by a [colon and a description that starts with a lowercase letter](colons.md). For non-code samples, remove the `code` elements—for example:
...
- Each item in the list follows our [list style](lists.md).
...
1.  In Cloud Shell, set the environment variables:
    ...
    - `ON_PREM_PROJECT_NAME`: the Google Cloud project name for your on-premises project. You can find your project number on the <a href="https://console.cloud.google.com/home/dashboard" target="console" data-track-name="consoleLink" data-track-type="how-to">Dashboard</a> page of the Google Cloud console.
