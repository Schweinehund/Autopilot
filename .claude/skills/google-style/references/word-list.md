...
_Source: https://developers.google.com/style/word-list_
...
If the term that you're looking for isn't on this list, check our other [editorial resources](https://developers.google.com/style#editorial-resources), including our preferred dictionary, [Merriam-Webster](https://www.merriam-webster.com/). If there are multiple spellings in the Merriam-Webster word entry, use the first form listed, which is the most common spelling. For example, in the [entry for *cancel*](https://www.merriam-webster.com/dictionary/canceled), the first form listed for the past tense is *canceled*, indicating that it's more common than *cancelled*.
...
As always, it's fine to deviate from our guidance if that serves your readers better. For more information, see [Break the rules](https://developers.google.com/style#rules).
...
- AVOID: **Use with caution**. A recommendation to avoid using the term *when possible*, or to use the term with caution. The term might be ambiguous or obscure, so we provide alternative term suggestions or suggest that you use a more specific term. However, you can use the term if needed. Where appropriate, define the term or use it only once, as explained on the [Jargon](jargon.md) page.
- DON'T USE: **Don't use**. In all cases, we prefer to *not use the term*. The term might be particularly ambiguous or it might have an offensive or non-inclusive association. If such a term appears in code, we recommend that you [replace or write around the term](inclusive-documentation.md#replace-or-write-around-non-inclusive-terms).
- Android: **Android**. Applies only to Android documentation.
- Cloud: **Google Cloud**. Applies only to Google Cloud documentation.
- Workspace: **Google Workspace**. Applies only to Google Workspace documentation.
...
2-Step Verification  
When referring to Google's [2-Step Verification](https://www.google.com/landing/2step/), use initial caps.

When referring to [generic 2-step verification](http://searchsecurity.techtarget.com/definition/two-step-verification), use lowercase.

### A

a and an  
Use *a* when the next word starts with a consonant *sound*, regardless of what letter it starts with. For more information, see [Articles (a, an, the)](articles.md).
...
: For more information about indexes, see [Managing indexes](https://cloud.google.com/firestore/docs/query-data/indexing).

: For more information on indexes, see [Managing indexes](https://cloud.google.com/firestore/docs/query-data/indexing).
...
Don't use to refer to a position in the UI. Instead, write instructions that avoid directional language. For more information, see [Writing accessible documentation](accessibility.md).
...
In Android documentation, don't use. Instead, use [*app bar*](#app-bar).
...
Android: It's OK to use *admin* in Android documentation.
...
: `PRODUCT_NAME` is in alpha.

AVOID: America, American  
Use only to refer to the *Americas* or the *American continent*.

Don't use to refer to the United States. Instead, use a more precise term like *the US* or *the United States*, and *people in the US*. For more information, see [US](#us).

among  
See [between versus among](#between).

AM, PM  
To be consistent with [Material Design](https://material.io/design/communication/data-formats.html#date-and-time), use all caps, no periods, and a space before.
...
AVOID: and/or  
Don't use unless space is limited, such as in a table. For more information, see [Slashes](slashes.md#and-or).
...
AVOID: and so on  
Avoid using *and so on* whenever possible. For more information, see [etc.](#etc)

anti\*  
See [guidance about hyphens with prefixes](hyphens.md#prefixes).
...
Don't use. Instead, refer to the *Google APIs Explorer* or to the *Google Cloud console*. For more information, see [console](#console).
...
For more information, see [Timeless documentation](timeless-documentation.md).
...
: `PRODUCT_NAME` is currently in beta.
...
See also [Grammar Girl's discussion of *between* and *among*](http://www.quickanddirtytips.com/education/grammar/between-versus-among).
...
: To deny requests from an IP address, add it to the `dos.yaml` file.

: To denylist an IP address, add it to the `dos.yaml` file.

: To blacklist an IP address, add it to the `dos.yaml` file.

If the command or code that you're documenting uses one of these words, then use the words only in direct reference to the code items ([formatted as code](code-in-text.md)), and make it clear what you're referring to.

: Add a user to the allowlist (`whitelist`) by entering the following: `whitelist adduser ``EMAIL_ADDRESS`.

: Add a user to the whitelist by entering the following: `whitelist adduser ``EMAIL_ADDRESS`.

For more information, see the [inclusive documentation](inclusive-documentation.md) page.
...
For information about clarifying who's performing an action, see [Active voice](voice.md).

AVOID: canary  
Don't use *canary* as a verb, and don't use *canarying*.

When possible, avoid [jargon](jargon.md) like *canary* and *canary testing*. If you use one of these phrases, define it on first use or provide a link to the definition, and use it consistently throughout the document.
...
choose  
*Choose* is fine to use for generic contexts. For UI elements, use [select](#select).
...
: Clear **Automatically check for updates**.

: Uncheck **Automatically check for updates**.

: Deselect **Automatically check for updates**.

AVOID: CLI  
Don't use *CLI* generically to refer to a command-line interface. Instead, refer to the specific command-line interface, such as the [Google Cloud CLI](#gcloud).
...
Android: For Android apps, don't use *click*. Instead, use [tap](word-list.md#tap).

DON'T USE: click here  
Don't use. For information and alternatives, see [Avoid vague link text](cross-references.md#vague-link-text).
...
Use *the* before the tool name. For more information, see [console](#console).
...
codelab  
Not *code lab* or *code-lab*. For more information, see [documentation](#documentation).

cold  
When possible, avoid [jargon](jargon.md) like *cold failover*, *cold standby*, and *cold spare*. If you use one of these phrases, define it on first use and use it consistently throughout the document.
...
confidential  
*Confidential* data is data that is protected to prevent unauthorized access. See [sensitive](#sensitive).

DON'T USE: cons  
Don't use. Instead, use a more precise term, such as *disadvantages*.

AVOID: console  
Don't use in isolation. Instead, use the name of the specific console, such as the [Google Cloud console](https://console.cloud.google.com/) or the Google Admin console.
...
AVOID: content type  
Be as specific as possible when writing about a content type, and use the term only when applicable. For example, you can use this term if you're referring to the value of the `Content-Type` HTTP header. Also see [media type](#media-type).

Control+S, Command+S, and other keyboard commands  
To refer to a  character, use +`CHARACTER`.

Don't use *Ctl-S*, *Cmd-S*, or *Cloverleaf-S*.

In most cases, use an uppercase letter for `CHARACTER`.

In macOS, many keyboard commands use the  key instead of the  key, and there's an  key instead of an  key. If your audience includes macOS users and Windows or Linux users, then mention both keyboard commands.
...
: In the **Query** field, enter the output from the previous step.

: Copy the output from the previous step and paste into the **Query** field.
...
For information about tenses, see [Present tense](tense.md).
...
DevOps  
Short for *development operations*. No need to spell out on first mention unless the audience requires it. For more information, see [DevOps](https://wikipedia.org/wiki/DevOps).
...
When describing a user action or the state of a UI element, use a more precise term where possible. You can use *inactive*, *unavailable*, *deactivate*, *turn off*, or *deselect*, depending on the context. Use the same term consistently throughout your document. See also [enable](#enable).
...
: The IAM recommender helps you *downscope* (reduce) the permissions that are available to your users.
...
Also don't use if referring to the concept in statistics known as a [dummy variable](https://en.wikipedia.org/wiki/Dummy_variable_(statistics)). Instead, use alternate terms such as *indicator variable*, *design variable*, *one-hot encoding*, *Boolean indicator*, *binary variable*, or *qualitative variable*.
...
Android: In Android documentation, don't use *earlier* for a range of version numbers. Instead, use *lower*.
...
Use a specific verb in front of the word. For example, *send email*. This construction is better for translation and a [global audience](translation.md).
...
enable  
In procedures, use the appropriate label and action for the [UI element](ui-elements.md) that the user interacts with. When describing a user action or the state of a UI element, use a more precise term where possible. It's OK to use *enable* when not referring to a person.

For turning on or activating an option or feature, use *enable* or *[turn on](#turn-on)* consistently:
...
Workspace: In Google Workspace documentation, if possible, use *turn on* or *on* instead. If referring to the state of a UI element, use *available*.

endpoint  
Not *end point*.

enter  
Use *enter* to refer to the user entering text. If it's important to not press , explicitly say so. See also [*type*](#type).

: In the **Owner** box, enter your name.

: In the **Size** box, type a font size.
...
Refer to *a FHIR* (pronounced "a fire," as in "a FHIR store"), not *an FHIR*. For more information, see [Indefinite articles before abbreviations](abbreviations.md#articles).
...
For more information, see [Write inclusive documentation](inclusive-documentation.md).

following  
It's not necessary to use a noun after *following* unless it helps provide clarity and enables accessibility. See [Tables](tables.md#table-placement).
...
: Enter a six-digit hex number, and then click **OK**. For example, if you want the color forest green, enter `228B22`.

For more information, see [Format examples](format-examples.md).

AVOID: for instance  
Don't use the phrase *for instance* to introduce examples to avoid confusion with the noun *instance*. Instead, use *for example*, *like*, or *such as*. For more information, see [for example](#for-example).
...
See also [eventually](#eventually) and [soon](#soon). For more information, see [Timeless documentation](timeless-documentation.md).

### G

GBps  
Short for *gigabytes per second*. By convention, we don't use *GB/s*. For more information, see [Units of measurement](units-of-measure.md).

Gbps  
Short for *gigabits per second*. By convention, we don't use *Gb/s*. For more information, see [Units of measurement](units-of-measure.md).
...
Don't use. Instead, use the singular *they* (see [Jane Austen and other famous authors violate what everyone learned in their English class](http://www.pemberley.com/janeinfo/austheir.html)). Don't use *he/she* or *(s)he* or other such punctuational approaches. For more information, see [Pronouns](pronouns.md).
...
Don't hyphenate *generative AI* as an adjective unless you must do so for clarity. See also [AI](#ai).
...
Google Play services SDK [](#google-play-services-SDK)  
Write *services* in lowercase.
...
Don't use. Follow the guidance for [black hat](#blackhat) when referring to someone violating rules or laws.
...
Don't use. Instead use the `aria-label` for that particular icon. For example, **Menu**. For more information, see [Buttons and icons](ui-elements.md#buttons).
...
DON'T USE: hang, hung  
Don't use to refer to a computer or system that is not responding. Instead, use *stop responding* or *not responding*. For more information, see [Avoid figurative language](inclusive-documentation.md#figurative-language).
...
The distinction the SRE documentation makes is between satisfying a need (a dispassionate act) and establishing an emotional response (creating happiness). Although it is difficult to measure happiness precisely, SRE uses [service level indicators (SLIs)](#service-level-indicator) to quantify user perception. For example, a customer might feel a "need" to watch a show on TV. If the show is available, the customer's need is satisfied. But if playback is slow or choppy, the customer might not be happy.
...
Spell as *high availability* when used as a noun and as *high-availability* when used as an adjective. See also [load balancing (noun), load-balancing (adjective)](#load-balancing).
...
Android: In Android documentation, use *higher* for a range of version numbers, not *later*.
...
: In the **Admin** menu, hover over **File**, and then click **New**.
...
hot  
When possible, avoid [jargon](jargon.md) like *hot failover*, *hot standby*, and *hot spare*. If you use one of these phrases, define it on first use and use it consistently throughout the document. However, see [hotspot](#hotspot).
...
DON'T USE: hover  
Don't use. Instead use [*hold the pointer over*](#hold-the-pointer-over).
...
Although it is common in casual usage to omit the word *then* in *if...then* statements, you should include helper words like *then* in technical documentation. For more information, see [Use clear, precise, and unambiguous language](translation.md#clear-language).
...
instance group  
Don't abbreviate to *IG*. See also [managed instance group](#mig).
...
: You can run DML statements in the same way that you'd run a `SELECT` statement.

: You can run DML statements just as you'd run a `SELECT` statement.
...
Sometimes, *just* is useful for conveying that one approach is simpler than another. In those cases, use *just* instead of [*simply*](#simple).

: Use the namespace ID `namespace:example-kind` or just `example-kind`.
...
KBps  
Short for *kilobytes per second*. By convention, we don't use *KB/s*. For more information, see [Units of measurement](units-of-measure.md).

Kbps  
Short for *kilobits per second*. By convention, we don't use *Kb/s*. For more information, see [Units of measurement](units-of-measure.md).

DON'T USE: kebab, kabob, kebab menu, kabob menu  
Don't use. Instead use the `aria-label` for that particular icon. For example, **More**. For more information, see [Buttons and icons](ui-elements.md#buttons).
...
AVOID: kill  
Avoid when possible. Instead, use words like *stop*, *exit*, *cancel*, or *end*. For exceptions to this rule, see [Documenting command-line syntax](code-syntax.md#linux-signals).
...
Android: In Android documentation, don't use *later* for a range of version numbers. Instead, use *higher*.
...
Don't use directional language. For more information, see [Writing accessible documentation](accessibility.md).

If referring to applications, use *[navigation menu](ui-elements.md#term-navigation-menu)*.
...
Spell as *load balancing* when used as a noun and as *load-balancing* when used as an adjective. See also [high availability (noun), high-availability (adjective)](#high-availability).
...
Android: In Android documentation, use *lower* for a range of version numbers, not *earlier*.
...
OK to abbreviate to *MIG* on subsequent mention. See also [instance group](#instance-group).
...
If the command or code that you're documenting uses the literal word *master*, then use this word only in direct reference to the code item ([formatted as code](code-in-text.md)), make it clear what you're referring to, and use the new term thereafter.
...
MBps  
Short for *megabytes per second*. By convention, we don't use *MB/s*. For more information, see [Units of measurement](units-of-measure.md).

Mbps  
Short for *megabits per second*. By convention, we don't use *Mb/s*. For more information, see [Units of measurement](units-of-measure.md).

media type  
In general, use the term [*media type*](https://www.iana.org/assignments/media-types/media-types.xhtml). In contexts where you need to refer to a *content type*—For example, if you mention the `Content-Type` HTTP header—it's okay to use *content type* instead, to avoid confusion. Don't use *MIME type*.
...
AVOID: MIME type  
*MIME* stands for "Multipurpose Internet Mail Extensions," and was originally used to refer to email standards. Don't use *MIME* when you mean [*media type*](https://www.iana.org/assignments/media-types/media-types.xhtml). If you feel that might be ambiguous to an audience familiar with the term *MIME*, then you can write *media (MIME) type* for clarity.
...
navigation bar  
Don't use to refer to a *navigation menu*. For more information, see [Navigation menu](ui-elements.md#term-navigation-menu).
...
Don't use *newer* to refer to a specific version of a product. Instead, use [*later*](#later). Make sure that you provide a version number or release date by which to understand *later*.

Android: In Android documentation, use [*higher*](#higher) instead of *later*.
...
For an instruction that does nothing, use [*no-op*](https://wikipedia.org/wiki/NOP_(code)) or the specific instruction name for your context.
...
AVOID: old, older  
Don't use to refer to a previous version of a product. Instead, use [*earlier*](#earlier).

Make sure that you provide a version number by which to understand *earlier*.

Android: In Android documentation, use [*lower*](#lower) instead of *earlier*.
...
per  
To express a rate, use *per* instead of the division slash (/), unless space constraints require the use of the slash. For more information, see [Units of measurement](units-of-measure.md#rates).
...
persist  
Don't use as a transitive verb. It's best to avoid using as a verb at all, especially in [passive voice](voice.md).
...
Don't use. Instead, use more precise terms like *persistent versus dynamic* or *manually configured versus automated*. For more information, see [Avoid figurative language](inclusive-documentation.md#figurative-language).
...
For more information, see [voice and tone](tone.md#politeness).
...
This is similar to the action [hold the pointer over (hover)](#hold-the-pointer-over). In most cases, it's better to use the verb phrase *hold the pointer over* if you want the user to wait for the UI to react.
...
To describe a window that appears and asks for, or presents, additional information, use [*dialog*](#dialog).
...
rehost  
Use to describe the migration of an app or workload with no changes or minimal changes to that app or workload. Also known as *lift and shift*. For more information, see [Rehost: lift and shift](https://cloud.google.com/architecture/migration-to-gcp-getting-started#rehost_lift_and_shift) in the Cloud Architecture Center.
...
SAP  
Pronounced as the individual letters *S*, *A*, *P*, so write *an SAP system*, not *a SAP system*. For more information, see [Indefinite articles before abbreviations](abbreviations.md#articles).
...
If you use *scroll*, don't use directional language like *scroll up*. For more information, see [Accessibility](accessibility.md#document-rendering).
...
see  
OK as a general term and when referring to links and cross-references. Our research indicates that language relating to sight is OK for a wide range of readers. For more information, see [Cross-references and linking](cross-references.md).
...
sensitive  
*Sensitive* data is data for which the release might be harmful. See [confidential](#confidential).
...
AVOID: shall  
Avoid *shall* except under advice from a lawyer. For more information, see [should](#should).
...
Because *should* is ambiguous by definition, it can be problematic. For more information and alternatives, see [Word choice for recommendations and requirements](prescriptive-documentation.md#word-choice).
...
single pane of glass <a href="#single-pane-of-glass" data-"=""></a>  
...
Don't use. Instead, use [*mobile phone*](#mobile) or *phone*. If you're talking about more than phones, then use *mobile device*. It's OK to use *phone* (without *mobile*) when the context is clear.
...
SQL  
Refer to *a SQL* (pronounced "a sequel"), not *an SQL*. For more information, see [Indefinite articles before abbreviations](abbreviations.md#articles).

ssh and SSH  
Don't use `ssh` or SSH as a verb. SSH is a secure communications protocol; `ssh` is a utility.

: To establish an SSH connection, use the `ssh` command.

: Connect to the instance by using SSH.

: `ssh` into your remote shell.

DON'T USE: ssh'ing  
Don't use. See also [ssh and SSH](#ssh).

: When you use `ssh` to log in ...
...
Avoid using [graphic or metaphorical language](inclusive-documentation.md#graphic-language). Instead, explain the relevant feature, such as *fence failed nodes*.
...
subnet  
OK to use as a shortening of *subnetwork*. Use the same term consistently throughout your document. For more information, see [Subnets vs. subnetworks](https://cloud.google.com/compute/docs/vpc/#subnets_vs_subnetworks).
...
such as  
Use *such as* to introduce examples or draw comparisons. Note that *such as*, *like*, and *include* introduce non-exhaustive lists, so it's redundant to combine them with *etc.*, *so forth*, or *and more*. See also [etc.](#etc), [like](#like). For more information, see [Format examples](format-examples.md).
...
tab  
When referring to the sub-pages of a [console](#console), use *page* instead of *tab*.
...
For a specific context where you can use *terminate* as a synonym for *stop*, see [Documenting command-line syntax](code-syntax.md#linux-signals).
...
Don't use. Instead, use *box*. For more information, see [Text box](ui-elements.md#term-textbox).

Cloud: In Google Cloud documentation, use *field* instead of *box*. For example, "In the **Instance** field, specify a value less than 64 characters long."

Workspace: In Google Workspace documentation, use *field* instead of *box*. For example, "In the **Instance** field, specify a value less than 64 characters long."
...
Avoid abbreviating to *3rd party* or *3rd-party*. For more information, see [Ordinal numbers](numbers.md#ordinal-numbers).
...
Spell as *time zone* when used as a noun and as *time-zone* when used as an adjective. See also [wake lock (noun), wake-lock (adjective)](#wake-lock).
...
AVOID: turn on  
In procedures, use the appropriate label and action for the [UI element](ui-elements.md) that the user interacts with.

For turning on or activating an option or feature, use *turn on* or [enable](#enable) consistently. Use the same term consistently throughout your document.
...
AVOID: type  
In general, use [enter](#enter) instead of *type* because there is typically more than one way to enter text than typing (such as pasting text or speaking).
...
UI  
Don't use generically to refer to a page or dashboard. Use a more specific term like [*page*](#page) or [*console*](#console). If a specific term is unavailable, use *web interface*.

: In the Google Cloud console

: On the **Cloud Tasks** page

: In the Secure Source Manager web interface

: In the **Cloud Tasks** UI
...
US  
OK to use as an abbreviation for *United States*. Don't use *U.S.* or *U.S.A.* For more information, see [Periods with abbreviations](abbreviations.md#periods).

user  
Use the word *user* only to refer to the user of the software that your reader is developing. Otherwise, address the reader as *you* and assume that they will complete the tasks that you're documenting. For more information, see [Second person and first person](person.md).
...
: You can filter for data with specific attributes by using custom filters.

: You can filter for data with specific attributes using custom filters.

UTF  
Include the hyphen in the names of Unicode encodings, such as *UTF-8*, *UTF-16*, and *UTF-32*.

<span icon="avoid"></span>utilize, utilization  
...
Cloud: For Google Cloud: on first mention of a Compute Engine VM, use *Compute Engine instance* and then use *compute instance* throughout the rest of the document. If you need to indicate other types of VMs, use *VM*, *VM instance*, or *bare metal instance*.
...
VLAN attachment  
Don't use the following: *interconnect attachment (VLAN)*, *Interconnect attachment*, *Cloud Interconnect attachment*, or any variation thereof. See also [interconnectAttachment](#interconnect-attachment).
...
Spell as *wake lock* when used as a noun and as *wake-lock* when used as an adjective. See also [time zone (noun), time-zone (adjective)](#time-zone).
...
warm  
When possible, avoid [jargon](jargon.md) like *warm failover*, *warm standby*, and *warm spare*. If you use one of these phrases, define it on first use and use it consistently throughout the document.
...
It's OK to use *we* to refer to the organization that's represented as the author of the document as long as the antecedent is clear. For more information, see [Second person and first person](person.md).
...
whether  
- To decide whether it's more appropriate to use *if* or *whether*, see [Grammar Girl's discussion of *if* and *whether*](http://www.quickanddirtytips.com/education/grammar/if-versus-whether).
- To decide whether you need to add *or not* when using *whether*, see [the New York Times's blog post about whether (or not)](http://afterdeadline.blogs.nytimes.com/2010/03/01/whether-or-not/).
...
will  
Avoid. Applies equally to its past tense, *would*. See also [Present tense](tense.md) and [Documenting future features](future.md).
...
you  
Use *you* instead of [*user*](#user) to address the reader of your document. For more information, see [Second person and first person](person.md).

### Z

AVOID: zippy  
Don't use to refer to [expander arrows](#expander-arrow), unless you're specifically referring to the [Zippy widget](https://google.github.io/closure-library/api/goog.ui.Zippy.html) in Closure.
