...
_Source: https://developers.google.com/style/abbreviations_
...
- A shortened word is just part of a word or phrase, sometimes with a period at the end:
  ...
  - *min* for *minutes*
  - *CA* for *California*
- [Contractions](contractions.md) are discussed in a separate page of this style guide.
...
- Spell out abbreviations on first reference. For more information, see [When to spell out a term](abbreviations.md#spelling-out).
...
- Avoid using abbreviations for terms that aren't related to the main topic of the document.
  ...
  : The internet of things (IoT) service can even be used for connecting to sensors in low Earth orbit.

  : The IoT (internet of things) service can even be used for connecting to sensors in LEO (low Earth orbit).

- Be wary of using specialized abbreviations that your readers might not understand. For more information about when to use such language, see [Jargon](jargon.md).
...
If the first mention of a term occurs in a [heading or title](headings.md), you can use the abbreviation and then spell out the abbreviation in the first paragraph that follows the heading or title.
...
- RAM
- REST
- [Units of measurement](units-of-measure.md#byte-units) such as MB, MiB, GB, or GiB
- URL
- USB
...
- Italicize both the spelled-out term and its abbreviation.
  ...
  : Establish *Border Gateway Protocol* (*BGP*) sessions using a router on the peer network.
...
Don't use *i.e.* or *e.g.*; instead, use *that is* or *for example*, respectively. For more information, see [e.g.](word-list.md#eg) and [i.e.](word-list.md#ie)

It's [okay to use *etc.* in some circumstances](word-list.md#etc), but it's best to use different phrasing in most lists. For more information, see [Comma-separated lists](lists.md#comma-separated-lists).

Don't use internet slang abbreviations such as [*tl;dr*](word-list.md#tldr), [*ymmv*](word-list.md#ymmv), [*RTFM*](word-list.md#rtfm), or others. Write out what you mean in a non-figurative way.
...
: Updating the software made throughput 10 times faster.

: Updating the software made throughput 10x faster.
...
- Put a period at the end of a shortened word, except for [date and time](dates-times.md) abbreviations.
...
For guidance about how to make abbreviations plural, see [Pluralization](pluralization.md#making-abbreviations-plural).

## Abbreviations as verbs

Don't use acronyms, initialisms, or shortened words as verbs.

: Use SSH to log in to your remote shell.

: Then ssh into your remote shell.
...
In particular, our word list includes preferences for "[a SQL](word-list.md#sql)", "[a FHIR](word-list.md#fhir)", and "[an SAP](word-list.md#sap)".

For more information about articles, see [Articles](articles.md).
